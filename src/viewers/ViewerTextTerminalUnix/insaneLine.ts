...
function numberPosition(p: blessed.Widgets.Position) {
    return {
        left: typeof p.left === 'string' ? parseInt(p.left) : p.left,
        right: typeof p.right === 'string' ? parseInt(p.right) : p.right,
        top: typeof p.top === 'string' ? parseInt(p.top) : p.top,
        bottom: typeof p.bottom === 'string' ? parseInt(p.bottom) : p.bottom,
    };
}
